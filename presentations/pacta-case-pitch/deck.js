...
document
  ...
  .forEach((button) =>
    button.addEventListener("click", () => render(Number(button.dataset.go))),
  );
...
document.addEventListener("pointerdown", (event) => {
  pointerStart = { x: event.clientX, time: performance.now() };
});
document.addEventListener("pointerup", (event) => {
  if (!pointerStart) return;
  const distance = event.clientX - pointerStart.x;
  const velocity =
    Math.abs(distance) / Math.max(1, performance.now() - pointerStart.time);
  if (Math.abs(distance) > 80 || velocity > 0.65)
